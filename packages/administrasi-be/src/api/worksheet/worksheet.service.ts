...
export class WorksheetService {
    ...
    async exportMemberWorkResult(res: Response, id: string) {
        ...
        worksheet.getCell('A1').alignment = { vertical: 'middle', horizontal: 'center' }; // Center alignment
        ...
        worksheet.getCell('A3').value = `Email ${getEnv('setting.env', 'COMPANY_EMAIL')} telphone ${getEnv('setting.env', 'COMPANY_PHONE')}`; // Set the title text
        worksheet.getCell('A3').font = { size: 14, bold: true }; // Set font size and bold
        worksheet.getCell('A3').alignment = { vertical: 'middle', horizontal: 'center' }; // Center alignment
        ...
        worksheet.getCell('A7').alignment = { vertical: 'distributed', horizontal: 'center' }; // Center alignment
        ...
        worksheet.getCell('A8').alignment = { vertical: 'distributed', horizontal: 'center' }; // Center alignment

        // Leave a blank row (row 6) for separation
        worksheet.addRow([]);

        // Define headers in row 7
        worksheet.getRow(9).values = ['NO', 'PETAK', 'LUAS', 'KEGIATAN', 'HARGA', 'QL', 'JUMLAH'];
        ...
        worksheet.getCell(`C${cellTotal}`).value = memberWorkResult.activities.reduce((prev, next) => prev + next.wide, 0)
        worksheet.getCell(`D${cellTotal}`).value = ''
        worksheet.mergeCells(`E${cellTotal}:F${cellTotal}`)
        worksheet.getCell(`E${cellTotal}`).value = 'TOTAL'
        worksheet.getCell(`G${cellTotal}`).value = formatRupiah(memberWorkResult.activities.reduce((prev, next) => prev + next.subTotal, 0))
        ...
        memberWorkResult.bon.map((item, index) => {
            worksheet.mergeCells(`E${initalCell + (index + 1)}:F${initalCell + (index + 1)}`);
            ...
            worksheet.getCell(`G${initalCell + (index + 1)}`).value = formatRupiah(item.total);
        ...
        worksheet.mergeCells(`E${initalCell + memberWorkResult.bon.length + 1}:F${initalCell + memberWorkResult.bon.length + 1}`);
        worksheet.getCell(`E${initalCell + memberWorkResult.bon.length + 1}`).value = 'TOTAL BON'; // Set the title text
        worksheet.getCell(`E${initalCell + memberWorkResult.bon.length + 1}`).font = { size: 12 }; // Set font size and bold
        worksheet.getCell(`G${initalCell + memberWorkResult.bon.length + 1}`).value = formatRupiah(memberWorkResult.bon.reduce((prev, next) => prev + next.total, 0));
        worksheet.getRow(initalCell + memberWorkResult.bon.length + 1).eachCell((cell) => {
            cell.fill = {
                type: 'pattern',
                pattern: 'solid',
        ...
        worksheet.mergeCells(`E${initalCell + memberWorkResult.bon.length + 2}:F${initalCell + memberWorkResult.bon.length + 2}`);
        worksheet.getCell(`E${initalCell + memberWorkResult.bon.length + 2}`).value = 'TOTAL BERSIH'; // Set the title text
        worksheet.getCell(`E${initalCell + memberWorkResult.bon.length + 2}`).font = { size: 12 }; // Set font size and bold
        worksheet.getCell(`G${initalCell + memberWorkResult.bon.length + 2}`).value = formatRupiah(memberWorkResult.activities.reduce((prev, next) => prev + next.subTotal, 0) - memberWorkResult.bon.reduce((prev, next) => prev + next.total, 0));
        worksheet.getRow(initalCell + memberWorkResult.bon.length + 2).eachCell((cell) => {
            cell.fill = {
                type: 'pattern',
                pattern: 'solid',
                fgColor: { argb: '37AFE1' } // Light gray background for headers
    ...
    async exportInvoiceActivity(res: Response, id: string) {
        ...
        worksheet.getCell('A8').alignment = { vertical: 'middle', horizontal: 'left' }; // Center alignment
        ...
        worksheet.getCell('A9').alignment = { vertical: 'middle', horizontal: 'left' }; // Center alignment
        ...
        worksheet.getCell('A10').alignment = { vertical: 'middle', horizontal: 'left' }; // Center alignment

        worksheet.getRow(12).values = ['NO', 'No. BAP/SPK', 'COMPT/ZONA', 'Activity', 'Luas Compt', 'Harga', 'Harga Jual/Penggantian/Uang Muka/Termin'];
        ...
        worksheet.getCell(`G${totalCell + 1}`).value = formatRupiah(invoice.invoiceActivities.reduce((prev, next) => prev + next.total, 0))
        ...
        worksheet.getCell(`A${totalCell + 2}`).value = `PPH ${getEnv('tax.env', 'TAX_AMOUNT')}%`
        ...
        worksheet.getCell(`G${totalCell + 2}`).value = formatRupiah((+getEnv('tax.env', 'TAX_AMOUNT') * invoice.invoiceActivities.reduce((prev, next) => prev + next.total, 0)) / 100)
        ...
        worksheet.getCell(`G${totalCell + 3}`).value = formatRupiah(invoice.invoiceActivities.reduce((prev, next) => prev + next.total, 0) - (+getEnv('tax.env', 'TAX_AMOUNT') * invoice.invoiceActivities.reduce((prev, next) => prev + next.total, 0)) / 100)
        ...
        worksheet.getCell(`A${totalCell + 4}`).value = `Terbilang : # ${numberToTextRupiah(invoice.invoiceActivities.reduce((prev, next) => prev + next.total, 0) - (+getEnv('tax.env', 'TAX_AMOUNT') * invoice.invoiceActivities.reduce((prev, next) => prev + next.total, 0)) / 100).toLocaleUpperCase()}`
        ...
        worksheet.getCell(`A${totalCell + 6}`).value = 'Kota Bangun, ' + moment(invoice.createdAt).format('DD MMMM YYYY')
        worksheet.getCell(`A${totalCell + 7}`).value = 'Hormat Kami'
        worksheet.getCell(`A${totalCell + 17}`).value = getEnv('setting.env', 'COMPANY_DIRECTURE');
        ...
        worksheet.getCell(`E${totalCell + 20}`).value = `: ${getEnv('setting.env', 'COMPANY_BANK_NAME')}`
        ...
        worksheet.getCell(`E${totalCell + 21}`).value = `: ${getEnv('setting.env', 'COMPANY_BANK_BRANCH')}`
        ...
        worksheet.getCell(`E${totalCell + 22}`).value = `: ${getEnv('setting.env', 'COMPANY_BANK_REKENING')}`
        ...
        worksheet.getCell(`E${totalCell + 23}`).value = `: ${getEnv('setting.env', 'COMPANY_BANK_OWNER')}`
    ...
    async exportInvoiceAdditional(res: Response, id: string) {
        ...
        worksheet.getRow(12).values = ['NO', 'No. BAP/SPK', 'Acitivy', 'BAP Amount DPP'];
        ...
        worksheet.getCell(`D${totalCell + 1}`).value = formatRupiah(invoice.invoiceAdditionals.reduce((prev, next) => prev + next.amount, 0))
        ...
        worksheet.getCell(`D${totalCell + 2}`).value = formatRupiah((+getEnv('tax.env', 'TAX_AMOUNT') * invoice.invoiceAdditionals.reduce((prev, next) => prev + next.amount, 0)) / 100)
        ...
        worksheet.getCell(`D${totalCell + 3}`).value = formatRupiah(invoice.invoiceAdditionals.reduce((prev, next) => prev + next.amount, 0) - (+getEnv('tax.env', 'TAX_AMOUNT') * invoice.invoiceAdditionals.reduce((prev, next) => prev + next.amount, 0)) / 100)
        ...
        worksheet.mergeCells(`A${totalCell + 4}:G${totalCell + 4}`)
        worksheet.getCell(`A${totalCell + 4}`).value = `Terbilang : # ${numberToTextRupiah(invoice.invoiceAdditionals.reduce((prev, next) => prev + next.amount, 0) - (+getEnv('tax.env', 'TAX_AMOUNT') * invoice.invoiceAdditionals.reduce((prev, next) => prev + next.amount, 0)) / 100).toLocaleUpperCase()}`
        worksheet.getCell(`A${totalCell + 4}`).font = {
            bold: true
        }
        worksheet.getRow(totalCell + 4).eachCell((cell) => {
