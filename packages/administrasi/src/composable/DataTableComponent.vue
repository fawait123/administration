...
const deleteSelectedusers = async () => {
    try {
        ...
        toast.add({ severity: 'success', summary: 'Opps!', detail: ResponseMessage.message(error), life: 3000 });
    }
};
...
<template>
    <div>
        <div class="card">
            <Toolbar class="mb-6" v-show="props.addButton == true || props.deleteButton == true || props.exportButton == true">
                <template #start>
                    <Button label="New" icon="pi pi-plus" severity="secondary" class="mr-2" @click="openNew" v-show="props.addButton" />
                    <Button label="Delete" icon="pi pi-trash" severity="secondary" v-show="props.deleteButton" @click="confirmDeleteSelected" :disabled="!selectedusers || !selectedusers.length" />
                </template>

                <template #end>
                    <Button label="Export" icon="pi pi-upload" severity="secondary" @click="exportCSV" v-show="props.exportButton" />
                </template>
            </Toolbar>
            ...
            <DataTable
                ...
                :page-link-size="users.pageLinkSize"
                :loading="users.loading"
                :total-records="users.totalRecords"
                v-on:page="onPagination"
            ...
            >
                ...
                <Column :field="column.field" :header="column.header" :sortable="column.sortable" style="min-width: 12rem" v-for="(column, i) in props.columns" :key="i">
