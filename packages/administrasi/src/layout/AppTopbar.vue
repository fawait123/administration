...
import { useCompanyStore, useProfileStore } from '@/store/index'
...
<template>
    <div class="layout-topbar">
        ...
        <div class="layout-topbar-actions">
            ...
            <div class="layout-topbar-menu hidden lg:block">
                <div class="layout-topbar-menu-content">
                    <Avatar image="https://primefaces.org/cdn/primevue/images/avatar/amyelsner.png"
                        class="layout-topbar-action w-[32px] h-[32px]" @click="toggle" />
                    <Menu ref="menu" id="overlay_menu" :model="items" :popup="true">
                        <template #submenuitem="{ item }">
                            <span class="text-primary font-bold">{{ item.label }}</span>
                        </template>
                        <template #item="{ item, props }">
                            <a v-ripple class="flex items-center" v-bind="props.action" @click="visible = true">
                        ...
                        <template #end>
                            <button v-ripple
                                ...
                                <span class="inline-flex flex-col items-start">
                                    <span class="font-bold">{{ profileStore.data.username }}</span>
                                    <span class="text-sm">{{ profileStore.data.role.name }}</span>
                                </span>
                            </button>
                        </template>
                    </Menu>
            ...
            <Dialog v-model:visible="visible" modal header="Keluar Sekarang" :style="{ width: '25rem' }">
                <span class="text-surface-500 dark:text-surface-400 block mb-8">Apakah anda yakin ingin keluar ?</span>
                <div class="flex justify-end gap-2">
                    <Button type="button" label="Batal" severity="secondary" @click="visible = false"></Button>
                    <Button type="button" label="Keluar" @click="logout"></Button>
                </div>
            </Dialog>
        </div>
    </div>
