...
const { validate: accountingValidate, isValid: accountingIsValid, getErros: accountGetErrors, getError: accountingGetError, scrolltoError: accountingScrolltoError } = useValidation(accountingSchema, invoiceRef, {
    mode: 'lazy',
});
                ...
                data: {
                    ...
                    details: detailAccounts.value.map((item) => {
                        return {
                            ...
                            subTotal: (item.percentage * total.value) /
                                100,
                            expenses: item.details.reduce((prev: any, next: any) => prev +
                                +next.amount, 0),
                            income: ((item.percentage * total.value) /
                                100) - (item.details.reduce((prev: any, next: any) => prev +
                                    +next.amount, 0)),
    ...
    <div>
        <div class="grid gap-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="card">
                    <CustomSelectGroup @onShow="onShow" :loading="loadingSelect" :editable="true" label="Pilih Invoice"
                        option-value="id" v-model="invoiceRef.invoiceId" :options="invoiceData" optionLabel="number"
                        :invalid="false" :error-message="``" placeholder="Pilih Invoice" class="w-full" />
                </div>
                <div class="card flex flex-col gap-6">
                    <h1 class="font-bold text-[16px] text-slate-500">{{ invoiceNumber }}</h1>
                    <h1 class="font-bold text-[24px] text-slate-500">{{ formatRupiah(total) }} / {{
                        formatRupiah(totalUsed) }}</h1>
                </div>
            </div>
            <div class="card">
                ...
                <DataTable :value="detailAccounts" tableStyle="min-width: 50rem" class="mt-4">
                    ...
                    <Column header="Sub Total">
                        <template #body="slotProps">
                            <span class="font-bold">{{ formatRupiah((slotProps.data.percentage * total) /
                                100) }}</span>
                        </template>
                    </Column>
                    <Column header="Pengurangan">
                        <template #body="slotProps">
                            <span class="font-bold">{{
                                formatRupiah(slotProps.data.details.reduce((prev: any, next: any) => prev +
                                    +next.amount, 0))
                            }}</span>
                        </template>
                    </Column>
                    <Column header="Pendapatan Bersih">
                        <template #body="slotProps">
                            <span class="font-bold">{{
                                formatRupiah(((slotProps.data.percentage * total) /
                                    100) - (slotProps.data.details.reduce((prev: any, next: any) => prev +
                                        +next.amount, 0)))
                            }}</span>
                        </template>
                    </Column>
        ...
        <Drawer v-model:visible="drawerAddAccount" header="Tambah Akun" position="right"
            class="min-w-[600px] max-w-[600px]">
            <div class="grid gap-4 mb-4">
                <div class="flex flex-col gap-8">
                    <CustomInputGroup label="Nama Akun" :error-message="getError('name')" :invalid="!!getError('name')"
                        v-model="accountRef.name" type="string" placeholder="Nama Akun" class="w-full" />
                </div>
                <div class="flex flex-col gap-8">
                    <CustomInputGroup label="Jumlah Persentase" :error-message="getError('percentage')"
                        :invalid="!!getError('percentage')" v-model="accountRef.percentage" type="number"
                        placeholder="Jumlah Persentase" class="w-full" />
                </div>
