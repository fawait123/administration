    ...
    <div>
        ...
        <div class="flex justify-center items-center" v-else>
            ...
            <Dialog v-model:visible="deleteActivity" :style="{ width: '450px' }" header="Hapus data" :modal="true">
                <span>Yakin ingin menghapus data (<span class="font-bold">Data didatabase akan ikut terhapus</span>) ?</span>
                <template #footer>
            ...
            <div class="max-w-[60%] min-w-[70%]">
                ...
                <div class="card">
                    <div class="grid grid-cols-2 gap-4">
                        <CustomSelectGroup
                            ...
                            label="Pilih Pekerja"
                            option-value="id"
                            v-model="formRef.employeeId"
                            :options="employeeData"
                            optionLabel="name"
                            :invalid="!!getError('employeeId')"
                            :error-message="getError('employeeId')"
                            placeholder="Pilih Pekerja"
                            class="w-full"
                        />
                        <CustomInputGroup type="date" placeholder="Masukan Periode" label="Periode" v-model="formRef.date" :invalid="!!getError('date')" :error-message="getError('date')" class-name="mb-8" />
                    </div>
                </div>
                ...
                <div class="card relative" v-for="(activity, i) in formRef.activities" :key="i">
                    <div class="grid grid-cols-3 gap-4">
                        <div class="flex flex-col gap-8">
                            <CustomInputGroup
                                label="Petak"
                                :invalid="!!getErros('activities', 'plot', i)"
                                :name="`plot[${[i]}]`"
                                :error-message="getErros('activities', 'plot', i)"
                                v-model="activity.plot"
                                type="text"
                                placeholder="Petak"
                                class="w-full"
                            />
                            <CustomSelectGroup
                                ...
                                :name="`activity[${[i]}]`"
                                :invalid="!!getErros('activities', 'activityId', i)"
                        ...
                        <div class="flex flex-col gap-8">
                            <CustomInputGroup
                                label="Luas"
                                :error-message="getErros('activities', 'wide', i)"
                                @input="calculateWide(activity.wide, i)"
                                :name="`wide[${[i]}]`"
                                :invalid="!!getErros('activities', 'wide', i)"
                                v-model="activity.wide"
                                type="number"
                                placeholder="Luas"
                                class="w-full"
                            />
                            <CustomInputGroup label="QL" :error-message="getErros('activities', 'ql', i)" :name="`ql[${[i]}]`" :invalid="!!getErros('activities', 'ql', i)" v-model="activity.ql" type="text" placeholder="QL" class="w-full" />
                        </div>
                        <div class="flex flex-col gap-8">
                            <CustomInputGroup
                                label="Harga"
                                :error-message="getErros('activities', 'price', i)"
                                @input="calculateprice(activity.price, i)"
                                :name="`price[${[i]}]`"
                                :invalid="!!getErros('activities', 'price', i)"
                            ...
                            <CustomInputGroup
                                label="Jumlah"
                                :error-message="getErros('activities', 'subTotal', i)"
                                :readonly="true"
                                :name="`subTotal[${[i]}]`"
                                :invalid="!!getErros('activities', 'subTotal', i)"
                                v-model="activity.subTotal"
                                type="text"
                                placeholder="Jumlah"
                                class="w-full"
                            />
                ...
                <div class="card relative" v-for="(bon, i) in formRef.bon" :key="i">
                    <div class="grid grid-cols-2 gap-4">
                        <div class="flex flex-col gap-8">
                            <CustomInputGroup label="Keterangan" :error-message="getErros('bon', 'note', i)" :name="`note[${[i]}]`" :invalid="!!getErros('bon', 'note', i)" v-model="bon.note" type="text" placeholder="Keterangan" class="w-full" />
                        </div>
                        <div class="flex flex-col gap-8">
                            <CustomInputGroup label="Total" :error-message="getErros('bon', 'total', i)" :name="`total[${[i]}]`" :invalid="!!getErros('bon', 'total', i)" v-model="bon.total" type="number" placeholder="Total" class="w-full" />
                        </div>
                    </div>
                    <div class="w-10 h-10 rounded-full bg-primary text-white flex justify-center items-center absolute top-[-10px] right-[-5px] cursor-pointer" @click="handlePlusMinusBon(i)" :class="i == 0 ? 'bg-primary' : 'bg-red-500'">
                        <span :class="i == 0 ? 'pi pi-plus' : 'pi pi-minus'"></span>
                    </div>
                </div>
