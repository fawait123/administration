    ...
    <div class="card">
        ...
        <div v-if="!store.loading">
            <ul class="p-0 mx-0 mt-0 mb-6 list-none grid grid-cols-1 gap-3" v-if="notificationRef?.result?.length > 0">
                <li class="flex items-center py-2 border-b border-surface" v-for="notification in notificationRef.result" :key="notification.id">
                ...
                <Paginator
                    v-model:first="first"
                    :rows="10"
                    :totalRecords="notificationRef?.count || 0"
